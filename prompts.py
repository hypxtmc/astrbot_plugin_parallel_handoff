...
SUBAGENT_OUTPUT_DISCIPLINE = (
    ...
    "⑦ 条数限量：结论最多 5 条。想说的比 5 条多，说明你还在铺背景——"
    "把背景全砍了，只留主代理还不知道的那几条。\n"
    "⑧ 禁排序式穷举：不写 P0/P1/P2，不写「一二三」「首先其次」，"
    "不按严重度排队。那是报告的形状，不是汇报的形状。\n"
)
...
SUBAGENT_DUAL_OUTPUT = (
    ...
    "一到三句，说人话。）\n"
    "【技术】\n"
    "（写给主代理看的那段。结论 + 证据（文件:行），压掉叙述——"
    "最多 5 条、总长 15 行以内。排优先级、写小标题、铺背景、"
    "把查的过程复述一遍，这几样都算写报告，不算汇报。"
    "只说主代理还不知道的。）\n"
    "两段都要写，但两段不许重叠——【日常】说结论和感受，"
    "【技术】说证据和细节，同一件事别在两段里各说一遍。"
    "用户只看得到【日常】，主代理只看得到【技术】。"
)
