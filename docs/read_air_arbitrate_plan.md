...
| **架空主代理显式意图** | `parallel_handoff` 的 `calls` 数组是主代理(普瑞赛斯)根据博士需求**显式选的人**，博士要的就是多角度并行对比 | V1 想「只取亲密度最高者」砍掉其他人=越权替主代理决定 |
...
而 **`parallel_handoff`（LLM 工具）不做仲裁砍人**——那是主代理在显式调度，博士要的就是多人并行，不该被抢。
...
【路径 B · LLM 工具】（parallel_handoff / call_subagent，调用前）
  ...
  - 温和收敛建议：当博士 UI 只点名一人、但 calls 误带多人时，日志提示主代理「本批是否只需某人」
    （仅建议，不强制过滤 calls）
...
3. **点名神圣不可侵**：T1 call_hits 强点名（含凯尔希/M3 爱称）仲裁绝不拦（延续 09-03 修复）
...
> 真实克制干预（返回 'main' 拦截短路）按博士 2026-09-03 12:31 拍板：**等三期随机性日常补齐后，再一起实验性开启测试**。
...
> 1) 更新 pending_batch（本批候选名单，供路径 A 读空气参考）；2) 温和收敛建议——博士只点名一人(call_t1_mentions 复用 RouterMixin)
...
> ② R4 嵌在 R2 的无条件 `return True` 之前，两条 return 结果相同、判定白算，是死代码；且判据键名 `presis` 与主代理真实记录键 `MAIN_SPEAKER("__main__")` 不一致，`c_main` 恒为 0。
...
> ⚠️ 旧测试 `test_read_air_quiet_on_old_rivalry` 用手工 `record("presis", ...)` 造数据，**测试绿但生产从不写入该键** —— 属"假测试掩盖真实缺陷"，已由 `test_read_air_r4_fires_with_real_main_key`（用真实键 `MAIN_SPEAKER`）覆盖补齐。
...
1. 点名凯尔希/M3 ← 依然准确直达（回归 09-03 修复）
2. 群聊杂谈/无关提问 ← 不再逢消息就抢派，主代理自然接住
3. 承接句 ← 平滑续给刚才的人，不断线
4. 多人并行 ← 博士显式要的并行照常，不因仲裁被砍
...
**② 旧怨的表现形式——贫嘴，不针锋相对**
- 关系网里的旧怨（如普瑞赛斯×凯尔希）只能体现在**贫嘴**上——刀子嘴豆腐心那种，嘴上贫、心里暖
...
*普瑞赛斯 V2 定稿，基于源码逐行核实的自迭代修正。关键改进：把「笼统仲裁层」拆成「双路径 + 共享状态」，抢话治理只放自动路由、绝不越权砍主代理显式调度，对齐 `_mode_shortcut_decision` 与 `_route_mem`。待博士确认。*