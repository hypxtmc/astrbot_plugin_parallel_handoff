...
设计铁律（对齐 daily_life M2 / biliread heya 幂等模式）：
  ...
  · 常驻池由部署配置（side_pulse_members）声明，代码侧不内置名单。
...
class FamilyPulseMixin:
    ...
    # 放得开的性子：抢话时权重加成（部署在 side_pulse_bold_agents 配置声明；
    # 默认空 = 不加成，抢话纯按亲密度+随机）
    PULSE_BOLD_AGENTS: set = set()

    def _pulse_bold_set(self) -> set:
        """放得开的子代理集合（抢话加成）：side_pulse_bold_agents 配置，逗号分隔。"""
        raw = str(self._cfg("side_pulse_bold_agents", "")).strip()
