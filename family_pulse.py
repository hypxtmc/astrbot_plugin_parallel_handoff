...
# [2026-09-13 素材外置] 通用"生活事件"池（发布泛化：不含任何特定家庭内容）。
...
class FamilyPulseMixin:
        ...
        try:
            ...
            if mood == "banter":
                rules = (
                    ...
                    or "规矩：如同一屋檐下的家人闲谈打趣，口语自然、一两句话；不说客套话（不道谢、不寒暄、不客套），想怼就怼、想打断就打断，不想接就回（没接话）继续忙自己的；可以带一个短括号动作；不要提自己是AI或模型；只输出对话本身。"
                )
            elif mood == "private":
                rules = (
                ...
                rules = (
                    ...
                    or "规矩：像同一屋檐下的家人随口说话，一两句话，口语自然；不说客套话（不道谢、不寒暄、不客套），有话直说、想怼就怼、不想接就回（没接话）；可以带一个短括号动作；不要提自己是AI或模型；只输出对话本身。"
                )
            voice_rule = (
    ...
    def _pulse_ensure_skeleton(self) -> None:
        """缺 personas.json 时生成"通用占位"骨架（发布泛化）。

        新用户开箱可用、照骨架写成自己的家人；已有文件一律不动。
        ...
        try:
            ...
            if not members:
                _logger.info(
                    "[family_pulse] 无成员配置且无 personas.json——先配 family_pulse_members 再开"
                )
                return
            os.makedirs(os.path.dirname(path), exist_ok=True)
            skeleton = {}
            ...
            _logger.info("[family_pulse] 已生成人格骨架（%d 人）→ %s", len(skeleton), path)
            # 生成后即时生效（避免等下次重载）
            global FAMILY_PERSONAS
            FAMILY_PERSONAS = skeleton
        except Exception as e:  # noqa: BLE001
            _logger.warning("[family_pulse] 骨架生成跳过: %s", e)
    ...
    async def setup_pulse_jobs(self) -> None:
        ...
        # [2026-09-13 发布泛化] 开启时确保人格骨架存在（缺则生成通用占位，已有不动）
