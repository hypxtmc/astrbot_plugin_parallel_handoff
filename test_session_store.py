...
class TestSessionStore:
    def test_append_load_roundtrip(self, tmp_path):
        ...
        assert store.append("closure", "umo:group:123", "user", "问题") is True
        assert store.append("closure", "umo:group:123", "assistant", "回答") is True

        hist = store.load("closure", "umo:group:123")
        assert len(hist) == 2
    ...
    def test_bad_line_skipped(self, tmp_path):
        """单行损坏只跳过该行，不影响其余历史。"""
        store = SessionStore(tmp_path)
        store.append("closure", "s1", "user", "好的")
        path = store._path("closure", "s1")
        with path.open("a", encoding="utf-8") as f:
            f.write("{这不是合法JSON\n")
            f.write("\n")  # 空行
        store.append("closure", "s1", "assistant", "另一条")

        hist = store.load("closure", "s1")
        assert len(hist) == 2
    ...
    def test_role_normalized(self, tmp_path):
        ...
        store = SessionStore(tmp_path)
        store.append("closure", "s2", "hacker", "内容")
        assert store.load("closure", "s2")[0]["role"] == "assistant"

    def test_prune_removes_old_only(self, tmp_path):
        store = SessionStore(tmp_path, retention_days=30)
        store.append("closure", "new", "user", "新的")
        store.append("closure", "old", "user", "旧的")

        # 把 old 的 mtime 改成 40 天前
        old_path = store._path("closure", "old")
        stale = time.time() - 40 * 86400
        os.utime(old_path, (stale, stale))

        removed = store.prune()
        assert removed == 1
        assert store.load("closure", "new") != []
        assert store.load("closure", "old") == []
    ...
    def test_same_session_different_agent_isolated(self, tmp_path):
        store = SessionStore(tmp_path)
        store.append("closure", "same", "user", "可露希尔的")
        store.append("amiya", "same", "user", "阿米娅的")
        assert store.load("closure", "same")[0]["content"] == "可露希尔的"
        assert store.load("amiya", "same")[0]["content"] == "阿米娅的"
...
class TestContextEnginePersist:
    def test_append_writes_disk(self, tmp_path):
        ...
        eng.append("closure", "sess", "问", "答")

        hist = store.load("closure", "sess")
        assert len(hist) == 2
    ...
    def test_inject_after_restart(self, tmp_path):
        ...
        eng1.append("closure", "sess", "第一问", "第一答")
        del eng1  # 模拟进程结束

        eng2 = ContextEngine(enabled=True, max_turns=10, store=store)
        import asyncio

        prompt, contexts = asyncio.run(eng2.inject("closure", "sess", "第二问"))
        assert prompt == "第二问"
    ...
    def test_lazy_load_not_eager(self, tmp_path):
        ...
        # 读一个从没写过的会话
        assert eng._ensure("closure", "never") == []
        assert eng.histories.get("closure:never") == []
    ...
    def test_store_none_falls_back_to_memory(self, tmp_path):
        ...
        eng.append("closure", "sess", "问", "答")
        assert len(eng.histories["closure:sess"]) == 2
        # 没有任何磁盘痕迹
    ...
    def test_memory_truncation_keeps_disk_full(self, tmp_path):
        ...
        for i in range(5):
            eng.append("closure", "sess", f"问{i}", f"答{i}")

        # 内存被截到 4 条
        assert len(eng.histories["closure:sess"]) == 4
        # 磁盘保留全量 10 条
        assert len(store.load("closure", "sess")) == 10

    def test_disabled_does_not_touch_disk(self, tmp_path):
        store = SessionStore(tmp_path)
        eng = ContextEngine(enabled=False, store=store)
        eng.append("closure", "sess", "问", "答")
        assert store.load("closure", "sess") == []
    ...
    def test_max_turns_window_applied_after_restart(self, tmp_path):
        ...
        eng1 = ContextEngine(enabled=True, max_turns=10, store=store)
        for i in range(8):
            eng1.append("closure", "sess", f"问{i}", f"答{i}")

        eng2 = ContextEngine(enabled=True, max_turns=2, store=store)
        import asyncio

        _, contexts = asyncio.run(eng2.inject("closure", "sess", "新问"))
...
class TestPruneWiring:
    ...
    def test_append_triggers_prune(self, tmp_path):
        ...
        assert store._path("closure", "new").exists(), "新文件不能误删"

    def test_prune_is_throttled(self, tmp_path):
        """24 小时内只清理一次——第二次 append 不再扫盘。"""
        store = SessionStore(tmp_path, retention_days=1)
        store._last_prune = 0.0
        store.append("closure", "a", "user", "触发清理")

        # 再造一个超期文件；此时节流窗口内，不该被清
        store.append("closure", "old2", "user", "y")
        p2 = store._path("closure", "old2")
        stale = time.time() - 5 * 86400
        os.utime(p2, (stale, stale))

        store.append("closure", "b", "user", "节流内不触发")
    ...
    def test_prune_failure_does_not_break_append(self, tmp_path, monkeypatch):
        ...
        assert store.append("closure", "s", "user", "内容") is True
