...
class DirectiveMixin:
    ...
    async def _route_directive_inject(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        - 无路由意图（纯闲聊）→ 不注入
        注入内容与顾主在 WebUI 填写的模式配置块保持一致，指令内明确标注本次任务分类。
        """
        try:
    ...
    # 顾主指定：T1/T2 小模型路由层与动态模式冲突，已关闭 enable_smart_router。
    ...
    # [2026-09-03 顾主指定修 bug] 技术对象正则：只有当消息表达"真正要干技术活"时才算 tech。
    # 复合技术特征词（对象/工具性动词），比 _TASK_TECH_RE 更严——剔除了 "查/检查/查询" 这类
    # 生活/身体/感情语境也常用的宽泛动词，避免"助手E，想检查肉棒成熟度吗？"里的"查"字
    ...
    # [2026-09-06 方案一·顾主指定] 点名场景只认硬技术【物】名词，剔除宽泛双义动词，治"点名贴贴被 relay 绕一圈"：
    ...
    def _classify_directive_task(self, event: AstrMessageEvent) -> str | None:
        ...
        if not message:
            return "affection"
        # [2026-09-03 顾主指定修 bug] 点名语义优先于 tech 关键词。
        # 原来先查 _TASK_TECH_RE，"检查"的"查"字命中 → 判 tech → 模式裁决放行主代理，
        # 把"助手E，想检查…"这种明确点名贴贴盖掉收卷回主代理。现在改为：
