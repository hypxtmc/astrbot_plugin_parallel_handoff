...
**双模式可选设置（v2.3.0，博士指定 2026-08-31）**：在 WebUI 配置两个 JSON 字符串块，自行决定每个模式的调度方式（数值随意填，改动即时生效，无需改代码）：
...
调用 `parallel_handoff` 时传 `mode="tech"` 或 `mode="affection"` 即按对应配置块调度；不传 mode 回落全局 `route_mode`/`call_mode`；显式传 `route_mode`/`call_mode` 参数优先于模式配置。与 T1/T2 小模型路由层的关系见「小模型路由层」章节——T1/T2 已由博士关闭（`enable_smart_router: false`），本模式配置走主代理路径，不受短路影响。
...
| 层 | 机制 | 成本 | 命中条件 |
|:---|:---|:---|:---|
| **T1 规则层** | 点名（中文名/英文 id + 词边界）、爱称别名（`T1_ALIASES`）、强领域词（`T1_KEYWORDS`，如"工程部"→closure、"画室"→xi） | 0 LLM 成本，毫秒级 | 单点名 + 非叙述语境 + 非技术文本 |
...
**内置保护（防误抢）：**
- **技术文本拦截**（T1）：报错/日志/代码块特征（traceback/exception/```/`code` 等）出现子代理名不视为点名，放行 main/T2——防 `module 'theresia' not found` 这类把日志里的名字当呼叫乱路由
- **叙述语境识别**（T1）："夕说过""昨天和黍聊了"是提及非呼叫；多点名歧义交 T2/main 仲裁
- **T2 默认归 main**：判向 prompt 明确"技术任务（默认）→ main"，无强相关给低分（0.1-0.3）
- **T1.5 三重约束**：纯承接句 + 时间窗 + 无新点名，宁可不接也不误路由

**与动态模式覆盖的分工**：T1/T2 命中 = 点名直达通道（direct 直发，主代理隐身，后宫贴贴场景）；未命中落主代理 = 主代理可用 `route_mode="relay"` 收卷汇总（技术干活场景）。边界：技术任务中若**主动点名**子代理（"让阿米娅整理这份资料"），T1 会短路直发——如需让此类消息落主代理收卷，可考虑 `router_tech_override`（规划中，未实现）。
...
- **双模式可选设置**：新增 `tech_mode_config` / `affection_mode_config` 两个博士可填配置块（JSON 字符串），各自独立定义 route_mode / call_mode / timeout，博士自行决定每个模式的调度方式；`parallel_handoff` 新增 `mode` 参数（tech/affection）按对应配置块调度，显式传参优先
