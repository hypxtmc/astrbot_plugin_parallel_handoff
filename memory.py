...
class MemoryMixin:
    ...
    # ── livingmemory 防腐层（2026-09-10 博士拍板）──────────
    ...
    async def _merge_pulse_memory_recall(
        ...
        """
        try:
            # 2026-09-12 博士指令：旁轨已关（enable_family_pulse=False），
    ...
    # 2026-09-12 博士拍板取消只读 —— 默认集补齐文件读写、语法/测试门禁与本地 git，
    ...
    def _subagent_event_stub(event, agent_name: str):
        ...
        # 2026-09-12 双写修复（博士拍板）：原先返回 1（非群聊），会触发
        ...
        #（实测 closure 会话 user 158 / assistant 78 ≈ 2:1，真人会话 1:1）。
