...
class RouterMixin:
    ...
    def _router_agent_pool(self) -> dict:
        ...
        [发布泛化 A 方案 2026-09-13] 未配置 router_tables.json（t2_brief 为空）时，
    ...
    def _discover_agent_pool(self) -> dict:
        """[发布泛化 A 方案 2026-09-13] 词表缺失时从编排器发现子代理，生成兜底路由池。
    ...
    # 格式：/张三 · /张三+王五 · /王五+赵六+钱七 · /主代理+张三+王五
    ...
    def _raw_command_text(self, event: AstrMessageEvent) -> str:
        ...
        bug 复现：用户发「/张三」，插件里 message_str 却是「张三」，T0 命令式
    ...
    def _is_explicit_transfer(self, message: str) -> bool:
        ...
        if any(w in m for w in self._TRANSFER_WORDS):
            return True
        # 句首点名：名字/爱称出现在开头且后接分隔标点（如「王五，你来接」）；
        # 名字作主语的长句叙述（「赵六刚才说的那个 bug…」）不算。
        for sep in ("，", "、", ",", "：", ":", " "):
            idx = m.find(sep)
    ...
    def _admin_reply_text(self, event, cmd: str) -> str:
        ...
        if cmd == "lock_main":
            # [主代理锁回执 2026-09-12 20:57 用户要求] 发 /主代理 后明确回报已锁定，
            # 而不是默默放行让消息落进主代理 LLM。
            return (
                "🔒 已锁定：主代理\n"
        ...
        if cmd == "lock_agents":
            # [命令锁回执 2026-09-13 用户要求] /名字 强锁明确回报已锁定，
    ...
    # 2026-08-30："又"收紧为必须后接叙述动词（"奸商又在坑我钱"是抱怨非叙述，"张三又做饭了"才是）；
    # 疑问句（吗/么/呢/吧/？）不视为叙述（"张三在画室吗"是询问非转述）
    ...
    # 使「张三，赵六，我们一起来玩吧」里"我们一起来"能命中"来"作拜请动词。
    ...
    def _t1_route(self, message: str):
        ...
        for name in sorted_names:
            ...
            if len(name_s) >= 2:
                ...
                # （"之前和张三聊了药膳" → "和张三"因词边界不相邻，此锚接住转述语境）
                cue_with = re.compile(
        ...
        # 原来要求名字在句首/有呼叫动词才命中，导致"帮我看一下赵六""最近怎么样"
    ...
    def _t1_route_multi(self, message: str):
        ...
        当消息里**明确强呼叫 ≥2 个子代理**（如「张三，赵六，我们一起来玩」）
        ...
        for name, aid in candidates.items():
            ...
            for m in pattern.finditer(stripped):
                ...
                if not call:
                    # 弱点名（句首/极短消息）也算强呼叫：如「张三，赵六，来」/「小黑，小白」
                    if not pre.strip() or (not post.strip() and len(stripped) <= 12):
                        call = True
                    else:
                        continue
    ...
    def _dedup_shortcircuit(
        ...
        2026-09-11：显式命令豁免——用户连发两条相同前缀命令（如 /张三）时，
        第二条必须照常执行，不能被去重窗口吃掉。
        """
    ...
    def _record_route_hit(self, event: AstrMessageEvent, agent: str) -> None:
        ...
        旧实现是并入（group[aid]=ts 不清旧成员），导致锁住一组后自然语言点「赵六」
        被 T1 当单点名追加进旧组，而不是换锁成赵六。用户心智：点名谁就是谁，
        ...
        for aid, cn in disp_map.items():
            for name in (aid, cn):
                ...
                if len(name_s) >= 2 and name_s.isascii():
                    ...
                    # 1) 叙述锚：说/聊过/提到…  2) 介词前导：和张三说…  3) 独立出现（后置仅排英文数字）
                    cue = re.compile(
                        rf"(?<![0-9A-Za-z\u4e00-\u9fff]){re.escape(name_s)}"
    ...
    async def _busy_bypass_check(self, event: AstrMessageEvent) -> bool:
        ...
        if cmd_agents and not cmd_main:
            # [命令锁回执 2026-09-13 用户要求] 同 smart 端：回执「🔒 已锁定：xxx」
        ...
        if cmd_main:
            ...
            if cmd_agents:
                ...
                return False
            # 纯 /主代理 → 建锁 + 回执并短路（2026-09-12 20:57 用户要求明确锁定标识）
            logger.info(
    ...
    async def _smart_router_check(self, event: AstrMessageEvent) -> bool:
        ...
        # 2026-09-11：显式命令豁免（连发两条 /张三，第二条必须照常执行）
        ...
        if cmd_agents or cmd_main:
            if cmd_agents and not cmd_main:
                # [命令锁回执 2026-09-13 用户要求] /名字 强锁改柜台式：
                # 回执「🔒 已锁定：xxx」+ 建锁 + 短路，对齐 /主代理 行为。
                # 旧行为「锁定并短路」会把命令消息直接转给子代理、触发其直接回复
                # （用户实测：没有锁定标识、命令被当对话落进子代理——严重 bug）。
                logger.info(
            ...
            if cmd_agents:
                ...
                # [主代理锁 2026-09-12 用户指定；回执强化 20:57 用户要求]
                # 纯 /主代理（或部署者主代理名）→ 建立会话级主代理锁 + 回执并短路，
                # 与 /谁在 同款柜台行为：明确回报「已锁定」，不再默默放行让消息
                # 落进主代理 LLM（用户原话：要报出已锁定 xxx 的标识消息）。
                logger.info(
        ...
        if self._PRESIS_TOKEN_RE and self._PRESIS_TOKEN_RE.search(message):
            logger.info(
