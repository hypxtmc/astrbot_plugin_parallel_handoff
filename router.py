...
class RouterMixin:
    ...
    def _raw_command_text(self, event: AstrMessageEvent) -> str:
        """还原含前缀的原始用户消息文本（2026-09-11 博士报 bug 治本）。

        bug 复现：博士发「/阿米娅」，插件里 message_str 却是「阿米娅」，T0 命令式
    ...
    def _is_explicit_transfer(self, message: str) -> bool:
        """本条是否构成「显式转移」（换人意图）——2026-09-11 博士口径。
        ...
        if any(w in m for w in self._TRANSFER_WORDS):
            return True
        # 句首点名：名字/爱称出现在开头且后接分隔标点（如「特蕾西娅，你来接」）；
        # 名字作主语的长句叙述（「斯卡蒂刚才说的那个 bug…」）不算。
        for sep in ("，", "、", ",", "：", ":", " "):
            idx = m.find(sep)
    ...
    def _parse_agent_command(self, message: str):
        """解析命令式触发。返回 (agents_list, has_presis)。

        agents_list: 命令点名的子代理 aid 列表（排除普瑞赛斯，普瑞赛斯是主代理本体）
        has_presis: 命令里是否含「普瑞赛斯」（标记主代理在场：不调度、放行主代理，
    ...
    # 使「阿米娅，斯卡蒂，我们一起来玩3p吧」里"我们一起来"能命中"来"作拜请动词。
    ...
    def _t1_route_multi(self, message: str):
        ...
        for name, aid in candidates.items():
            ...
            for m in pattern.finditer(stripped):
                ...
                if not call:
                    # 弱点名（句首/极短消息）也算强呼叫：如「阿米娅，斯卡蒂，来」/「兔兔，猫猫」
                    if not pre.strip() or (not post.strip() and len(stripped) <= 12):
                        call = True
                    else:
                        continue
    ...
    def _dedup_shortcircuit(
        ...
        2026-09-11：显式命令豁免——博士连发两条相同前缀命令（如 /阿米娅）时，
        第二条必须照常执行，不能被去重窗口吃掉。
        """
    ...
    def _t1_sticky_route(self, event: AstrMessageEvent, message: str):
        ...
        或消息含「普瑞赛斯」最高级令牌（由上层先行清锁，本层不接管）。

        取代旧 T1.5 的「纯承接短句 + 300s 时间窗 + 无新点名」弱续接设计，
        按博士 2026-09-07 指定改为会话级硬锁定（agent → 一路粘着，久聊不解放绑）。
        ...
        #    长句里顺带提及则继续沿用旧锁（2026-09-11 博士口径：锁不被 T1 夺走）
    ...
    def _mode_shortcut_decision(self, event, message: str, route: str) -> bool:
        ...
        裁决规则（博士配置永远优先；2026-09-07 方案A 调整 tech 分支）：
        - 任务分类 tech（技术特征）+ 路由命中（点名/粘滞）→ **短路直发被点名者处理**
          （博士 2026-09-07 拍板：点名粘滞期间技术请求也归被点名子代理直发处理，
    ...
    async def _busy_bypass_check(self, event: AstrMessageEvent) -> bool:
        ...
        # [T0 命令式触发 2026-09-08 博士指定] busy 场景同样启用 / 命令：
        ...
        if cmd_presis:
            ...
            logger.info(
                f"[parallel_handoff] BusyBypass: T0 命令式含普瑞赛斯 → 放行主代理 "
    ...
    async def _smart_router_check(self, event: AstrMessageEvent) -> bool:
        ...
        # 2026-09-11：显式命令豁免（连发两条 /阿米娅，第二条必须照常执行）
        ...
        if cmd_agents or cmd_presis:
            ...
            if cmd_agents:
                ...
                logger.info(
                    "[parallel_handoff] SmartRouter: T0 命令式 /普瑞赛斯 → 纯主代理放行"
                )
                last, _ = self._route_mem()
        ...
        if self._PRESIS_TOKEN_RE.search(message):
            ...
            # [2026-09-11 博士口径修正] 这里**不再**清命令锁：过去对话里只要提到
            # 「普瑞赛斯」四个字就把命令强锁拆了，是「锁莫名其妙失效」的元凶之一。
            # 现在令牌只放行本轮；要解绑请发 /普瑞赛斯（走 T0 命令路径显式解锁）。
            last, _ = self._route_mem()
            last.pop(event.unified_msg_origin, None)
            return False
        ...
        # T1 点名 / 领域词
        route = self._t1_route(message)
        # [锁仲裁 2026-09-11 博士口径] 粘滞在册组存在时，T1 只有提名权、没有转移权：
